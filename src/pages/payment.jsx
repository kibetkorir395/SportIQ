...
export default function Payment({ showNotification, showModal }) {
  ...
  const startPaymentPolling = (reference, amount) => {
    ...
    showModal({
      ...
      message: `Please check your phone and enter M-Pesa PIN to complete payment of KSh ${amount}`,
      confirmText: 'OK',
      showCancel: false
    });
    ...
    const interval = setInterval(async () => {
      if (attempts >= maxAttempts) {
        ...
        showModal({
          ...
          message: 'Payment monitoring timeout. Please check your transaction history or try again.',
          confirmText: 'OK',
          showCancel: false
        });
        return;
        ...
        showModal({
          ...
          message: (
            <div style={{ textAlign: 'center' }}>
              ...
              <h3 style={{ margin: '10px 0', color: '#166534' }}>KSh {selectedPricing.price} Paid</h3>
              <p>Your {selectedPricing.period}ly subscription has been activated.</p>
              <p style={{ fontSize: '12px', color: '#666' }}>Reference: {reference}</p>
            </div>
          ),
          confirmText: 'View Predictions',
    ...
    <div className="payment-container" style={{marginTop: "70px"}}>
      ...
      {pollingReference && (
        <div className="payment-status" style={{
          ...
          <i className="fas fa-spinner fa-spin" style={{ marginRight: '10px', color: '#856404' }}></i>
          <span style={{ color: '#856404' }}>Waiting for payment confirmation...</span>
        </div>
      )}
    </div>
  );
